...
def fit(location, data, uncertainty = None, use_uncertainty=False):
    p0 = [trapz(data*location, location)/trapz(data, location),
            abs(trapz(data, location)), 
            10.0e-3, 
            5.0e-3, 
            max(1.0, data.mean())
        ]
    ...
    def likelihood(p):
        if any(p<pMin) or any(p>pMax):
            return 99e99
        if use_uncertainty:
            result = Eich_model(p, location)/uncertainty - data/uncertainty
        else:
            result = Eich_model(p, location) - data
        result *= result
        result = sum(result)
